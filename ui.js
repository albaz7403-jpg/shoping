...
function filterProducts() {
    ...
    if (grid) grid.innerHTML = filtered.map(p => productCardHTML(p)).join('');
}
    ...
    app.innerHTML = `
        <div class="page-container">
            ...
            <div class="upload-section" id="uploadSection" onclick="document.getElementById('videoFileInput').click()">
                ...
                <button class="upload-btn"><i class="fas fa-cloud-upload-alt"></i> اختر فيديو</button>
                <input type="file" id="videoFileInput" accept="video/*" style="display: none;" onchange="handleVideoUpload(event)" capture="environment">
            ...
            <div id="uploadedVideoPreview" style="margin-top: 16px; display: none;">
                ...
                <div class="video-card" style="max-width: 500px; margin: 0 auto;">
                    <div class="video-wrapper"><video id="videoPreview" controls></video></div>
                    <div class="video-info">
                        <div class="form-group"><label>عنوان الفيديو</label><input type="text" id="videoTitle" class="form-input" placeholder="أدخل عنواناً للفيديو"></div>
                        <div class="form-group"><label>وصف (اختياري)</label><input type="text" id="videoDescription" class="form-input" placeholder="وصف مختصر"></div>
                        <button class="btn btn-primary btn-block" onclick="saveUploadedVideo()"><i class="fas fa-save"></i> حفظ الفيديو</button>
                    </div>
                </div>
            </div>
            
            <div class="text-center mt-2">
                <button class="btn btn-secondary" onclick="navigateTo('home')"><i class="fas fa-arrow-right"></i> العودة للرئيسية</button>
            </div>
        </div>
    `;
...
function renderStaticPage(title, content) {
    ...
    app.innerHTML = `
        <div class="page-container text-center" style="max-width: 700px; margin: 40px auto;">
            ...
            <button class="btn btn-primary mt-2" onclick="navigateTo('home')"><i class="fas fa-home"></i> العودة للرئيسية</button>
        </div>
    `;
}
...
function productCardHTML(p) {
    ...
    return `
        <div class="product-card" id="product-${p.id}">
            ${isWishlisted ? `<div class="wishlist-btn active" onclick="toggleWishlist('${p.id}')"><i class="fas fa-heart"></i></div>` : `<div class="wishlist-btn" onclick="toggleWishlist('${p.id}')"><i class="far fa-heart"></i></div>`}
            ...
            <div class="stock-info ${p.stock <= 0 ? 'out' : p.stock <= 3 ? 'low' : ''}">${p.stock > 0 ? `✅ ${p.stock} قطعة متاحة` : '❌ غير متوفر'}</div>
            ${sizes.length > 0 ? `<div class="size-options" id="sizes-${p.id}">${sizes.map((s, i) => `<button class="size-btn ${i === 0 ? 'selected' : ''}" onclick="selectSize('${p.id}', '${s}', this)">${s}</button>`).join('')}</div>` : ''}
            <div class="quantity-control">
                <button class="qty-btn" onclick="changeQty('${p.id}', -1)">-</button>
                <span class="qty-value" id="qty-${p.id}">1</span>
                <button class="qty-btn" onclick="changeQty('${p.id}', 1)">+</button>
            </div>
            <button class="add-to-cart-btn" onclick="addToCart('${p.id}')" ${isOutOfStock ? 'disabled' : ''}>${isOutOfStock ? 'غير متوفر' : '🛒 أضف للسلة'}</button>
        </div>
    `;
}
    ...
    return `
        <div class="video-card" id="video-${video.id}">
            <div class="video-wrapper">
                ${isYouTube ? `<iframe src="https://www.youtube.com/embed/${video.youtubeId}?rel=0" title="${video.title}" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>` : ''}
                ${isLocal ? `<video controls preload="metadata" poster="${video.thumbnail || ''}"><source src="${video.url}" type="video/mp4">متصفحك لا يدعم تشغيل الفيديو</video>` : ''}
            </div>
            <div class="video-info">
                <h3>${video.title}</h3>
            ...
            ${(currentUser === 'admin' || currentUser === 'developer') ? `<div class="video-actions"><button class="btn btn-danger btn-sm btn-block" onclick="deleteVideo('${video.id}')"><i class="fas fa-trash"></i> حذف</button></div>` : ''}
        </div>
    `;
}
    ...
    const formHTML = `
        <div class="modal active" id="addVideoModal" onclick="if(event.target === this) this.remove()">
            <div class="modal-content">
                <div class="modal-header"><h2>➕ إضافة فيديو جديد</h2><button class="modal-close" onclick="document.getElementById('addVideoModal').remove()">&times;</button></div>
                
                <div class="admin-section">
                    <h3>🎬 إضافة من يوتيوب</h3>
                    <div class="form-group"><label>رابط فيديو يوتيوب</label><input type="text" id="youtubeUrl" class="form-input" placeholder="https://www.youtube.com/watch?v=... أو https://youtu.be/..."></div>
                    <div class="form-group"><label>عنوان الفيديو</label><input type="text" id="youtubeTitle" class="form-input" placeholder="عنوان وصفي للفيديو"></div>
                    <div class="form-group"><label>وصف (اختياري)</label><input type="text" id="youtubeDescription" class="form-input" placeholder="وصف مختصر"></div>
                    <button class="btn btn-primary btn-block" onclick="addYouTubeVideo()"><i class="fab fa-youtube"></i> إضافة فيديو يوتيوب</button>
                </div>
                
                <div class="admin-section" style="margin-top: 16px;">
                    <h3>📁 أو ارفع فيديو من جهازك</h3>
                    <p style="font-size: 0.85rem; color: var(--gray); margin-bottom: 12px;">يمكنك رفع فيديو مباشرة من هاتفك أو كمبيوترك</p>
                    <input type="file" id="adminVideoFile" accept="video/*" class="form-input" onchange="handleAdminVideoSelect(event)">
                    <div id="adminVideoPreview" style="margin-top: 12px; display: none;">
                        ...
                        <div class="form-group" style="margin-top: 12px;"><label>عنوان الفيديو</label><input type="text" id="adminVideoTitle" class="form-input" placeholder="عنوان الفيديو"></div>
                        <button class="btn btn-success btn-block" onclick="saveAdminVideo()"><i class="fas fa-save"></i> حفظ الفيديو</button>
                    </div>
                </div>
                
                <button class="btn btn-secondary btn-block btn-sm mt-2" onclick="document.getElementById('addVideoModal').remove()">إلغاء</button>
            </div>
        </div>
    `;
...
async function addYouTubeVideo() {
    ...
    if (!url) { showToast('⚠️ الرجاء إدخال رابط الفيديو', 'error'); return; }
    
    const youtubeId = extractYouTubeId(url);
    if (!youtubeId) { showToast('⚠️ رابط يوتيوب غير صالح', 'error'); return; }
    if (!title) { showToast('⚠️ الرجاء إدخال عنوان الفيديو', 'error'); return; }
    
    const video = { type: 'youtube', youtubeId, title, description: description || '', thumbnail: `https://img.youtube.com/vi/${youtubeId}/hqdefault.jpg`, url: `https://www.youtube.com/watch?v=${youtubeId}` };
...
async function saveUploadedVideo() {
    ...
    if (!file) { showToast('⚠️ الرجاء اختيار فيديو أولاً', 'error'); return; }
    if (!title) { showToast('⚠️ الرجاء إدخال عنوان للفيديو', 'error'); return; }
    
    const videoUrl = URL.createObjectURL(file);
    const video = { type: 'local', title, description: description || '', url: videoUrl, fileName: file.name, fileSize: file.size, fileType: file.type };
...
async function saveAdminVideo() {
    ...
    if (!file) { showToast('⚠️ الرجاء اختيار فيديو', 'error'); return; }
    if (!title) { showToast('⚠️ الرجاء إدخال عنوان', 'error'); return; }
    
    const videoUrl = URL.createObjectURL(file);
    const video = { type: 'local', title, description: '', url: videoUrl, fileName: file.name, fileSize: file.size, fileType: file.type };
...
function updateUIForUser() {
    ...
    if (userInfo) {
        if (isAdmin) {
            userInfo.innerHTML = `<div class="user-avatar"><i class="fas fa-crown"></i></div><div><div style="font-weight: 700;">${isDev ? 'مطور المتجر' : 'مدير المتجر'}</div><div style="font-size: 0.75rem; color: var(--success);">مسجل الدخول</div></div>`;
        } else if (isLoggedIn) {
            userInfo.innerHTML = `<div class="user-avatar"><i class="fas fa-user"></i></div><div><div style="font-weight: 700;">${currentUser.name || 'مستخدم'}</div><div style="font-size: 0.75rem; color: var(--success);">مسجل الدخول</div></div>`;
        }
    }
...
function showToast(message, type = 'info') {
    ...
    toast.style.background = type === 'success' ? 'var(--success)' : type === 'error' ? 'var(--danger)' : '#333';
...
function openWishlistModal() {
    ...
    if (wishlist.length === 0) {
        ...
        list.innerHTML = wishlist.map(p => `<div class="cart-item"><div class="cart-item-info"><div class="cart-item-name">${p.name}</div><div class="cart-item-price">${p.price} ${storeInfo.currency}</div></div><div class="cart-item-controls"><button class="btn btn-primary btn-sm" onclick="addToCart('${p.id}'); closeModal('wishlistModal')">أضف للسلة</button><button class="cart-remove-btn" onclick="toggleWishlist('${p.id}'); openWishlistModal()"><i class="fas fa-trash"></i></button></div></div>`).join('');
    }
...
function performSearch() {
    ...
    if (!query) { navigateTo('products'); return; }
    
    const filtered = products.filter(p => p.name.toLowerCase().includes(query) || (p.description && p.description.toLowerCase().includes(query)) || p.category.toLowerCase().includes(query));
    
    const app = document.getElementById('app');
    app.innerHTML = `<div class="page-container"><div class="search-section"><div class="search-box"><input type="text" id="searchInput" placeholder="🔍 ابحث عن منتج..." value="${query}"><button onclick="performSearch()"><i class="fas fa-search"></i> بحث</button></div></div><h2 class="section-title">🔍 نتائج البحث عن "${query}"</h2><div class="products-grid">${filtered.length > 0 ? filtered.map(p => productCardHTML(p)).join('') : '<p class="text-center">لا توجد نتائج</p>'}</div><button class="btn btn-secondary mt-2" onclick="navigateTo('home')"><i class="fas fa-arrow-right"></i> العودة</button></div>`;
}
...
async function submitComplaint() {
    ...
    if (!text) { showToast('⚠️ الرجاء كتابة الشكوى', 'error'); return; }
    
    await addDocument('complaints', { name: name || 'مجهول', phone: phone || 'غير محدد', text, status: 'new' });
...
// ============================================
// دوال الشريط الإعلاني وفيديو الصفحة الرئيسية
// ============================================

const videoSettings = {
    adYoutubeId: 'https://youtube.com/shorts/2th0c0utuco?si=Rn4XiEX2Bp1ioxYG',        // ← غير هذا بمعرف فيديو الإعلان
    adDuration: 20,                      // مدة الإعلان بالثواني
    adShowDelay: 3,                      // تأخير الظهور بالثواني
    heroYoutubeId: 'M7lc1UVf-VE',       // ← غير هذا بمعرف فيديو الصفحة الرئيسية
    heroHeight: 350,
    heroShow: true
};
...
function showVideoBanner() {
    ...
    frame.src = 'https://www.youtube.com/embed/' + videoSettings.adYoutubeId + '?autoplay=1&mute=1&controls=0&loop=0&rel=0&showinfo=0&modestbranding=1';
    banner.classList.add('active');
    
    let timeLeft = videoSettings.adDuration;
    if (timerEl) timerEl.textContent = timeLeft;
    
    bannerTimerInterval = setInterval(function() {
        timeLeft--;
        if (timerEl) timerEl.textContent = timeLeft;
        if (timeLeft <= 0) closeVideoBanner();
    }, 1000);
}
...
function renderHeroVideo() {
    if (!videoSettings.heroYoutubeId || !videoSettings.heroShow) return '';
    return '<div class="hero-video-section" style="height: ' + videoSettings.heroHeight + 'px;"><iframe src="https://www.youtube.com/embed/' + videoSettings.heroYoutubeId + '?autoplay=1&mute=1&loop=1&playlist=' + videoSettings.heroYoutubeId + '&controls=1&rel=0&showinfo=0" allow="autoplay; encrypted-media" allowfullscreen></iframe></div>';
}

// تشغيل الإعلان بعد تحميل الصفحة
setTimeout(function() { showVideoBanner(); }, videoSettings.adShowDelay * 1000);

// إعادة الإعلان كل 3 دقائق
setInterval(function() {
    const banner = document.getElementById('videoBanner');
    if (!banner || !banner.classList.contains('active')) showVideoBanner();
}, 180000);